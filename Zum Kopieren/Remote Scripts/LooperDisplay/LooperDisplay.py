...
Bedienung pro Kanalstreifen 1-8 (bezogen auf die aktuelle 8er-Bank):
    ...
    S  (CC 32-39)  Stop -- NICHT geforwardet, in Live auf den Stop-Knopf
                   des Loopers gemappt, damit dessen Quantisierung greift
...
CC_SOLO_BASE = 32         # S 32..39  -> NICHT geforwardet (Stop via Live-Mapping)
...
#   /nano/looper     <n> <spurname> <state>              pro Looper, bei Aenderung, alle 2 s
...
class LooperDisplay(ControlSurface):
    ...
    # S (32-39) fehlt hier absichtlich -> geht an Lives MIDI-Mapping.
    # Fader, Regler und Transport bleiben ebenfalls frei fuer Live.
    # ------------------------------------------------------------------
    def build_midi_map(self, midi_map_handle):
        ...
        for index in range(NUM_STRIPS):
            # R, S und M fehlen hier absichtlich: Sie gehen an Lives
